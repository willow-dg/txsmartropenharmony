...
#define SERVO_ALT_PORT  EPWMDEV_PWM2_M1
#define GPIO_EMERG      GPIO0_PA5

#if FEATURE_SERVO
#if SERVO_REUSE_VALVE_PWM
#define SERVO_PORT      VALVE_PORT
#else
#define SERVO_PORT      SERVO_ALT_PORT
#endif
#endif

#if FEATURE_SERVO && FEATURE_ULTRASONIC && SERVO_REUSE_VALVE_PWM
#warning "三路同时演示请把 SERVO_REUSE_VALVE_PWM 改为 0：舵机 PWM2/PB6，电机 PWM6，超声 Trig PA2"
#endif
...
void lifeline_alarm_init(void)
{
    ...
    beep_pwm_set(0);
#if FEATURE_SERVO
    IoTPwmInit(SERVO_PORT);
    IoTPwmStart(SERVO_PORT, SERVO_DUTY_OPEN, SERVO_PWM_FREQ);
#if !SERVO_REUSE_VALVE_PWM
    IoTPwmInit(VALVE_PORT);
    pwm_channel_set(VALVE_PORT, 0);
#endif
    printf("servo PWM %s 50Hz (valve %s, ultra Trig=PA2)\n",
           SERVO_REUSE_VALVE_PWM ? "PWM6" : "PWM2_M1/PB6",
           SERVO_REUSE_VALVE_PWM ? "replaced" : "motor PWM6 kept");
#else
    IoTPwmInit(VALVE_PORT);
...
void valve_set_state(bool state)
{
    if (state != s_valve_logic)
    {
        s_valve_logic = state;
#if FEATURE_SERVO
        IoTPwmStart(SERVO_PORT, state ? SERVO_DUTY_OPEN : SERVO_DUTY_CLOSE,
                    SERVO_PWM_FREQ);
        s_valve_actuate_ticks = 0;
#if !SERVO_REUSE_VALVE_PWM
...
{
#if FEATURE_SERVO && SERVO_REUSE_VALVE_PWM
    (void)s_valve_actuate_ticks;
    return;
#else
