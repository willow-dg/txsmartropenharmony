...
/*
 * verbose=1：EEPROM/SRAM 页读写失败要打 Error（真总线问题）。
 * verbose=0：轮询/探卡时的预期 NACK（RF 占锁、无场）保持安静。
 */
static bool readTimeoutEx(uint8_t address, uint8_t *block_data, unsigned int len, int verbose)
{
    uint32_t status = 0;
    ...
    status = IoTI2cRead(NFC_I2C_PORT, NT3H1X_SLAVE_ADDRESS, block_data, len);
    if (status != IOT_SUCCESS)
    {
        if (verbose)
        {
...
{
    return readTimeoutEx(address, block_data, NFC_PAGE_SIZE, 1);
}
...
/*
 * 会话寄存器不是 16 字节 EEPROM 页（c4_nfc 按页读 0xFE 会 NACK）。
 * 1201 / plus 都是：写 [0xFE][sess_addr] 再读 1 字节（datasheet 9.8）。
 * 无场可读；RF 占锁时常 NACK，上层当有场，此处不打 Error。
 * NS_REG 位序见 NT3H.h：本板 1201 与 plus 相反。
 */
bool getSessionReg(void)
{
    uint8_t cmd[2] = { SESSION_REG, SESSION_NS_REG };
    uint8_t ns = 0;
    uint32_t status;

    memset(nfcPageBuffer, 0, NFC_PAGE_SIZE);

    status = IoTI2cWrite(NFC_I2C_PORT, NT3H1X_SLAVE_ADDRESS, cmd, 2);
    if (status != IOT_SUCCESS)
    {
        return 0;
    }

    status = IoTI2cRead(NFC_I2C_PORT, NT3H1X_SLAVE_ADDRESS, &ns, 1);
    if (status != IOT_SUCCESS)
    {
        return 0;
    }
...
static bool NT3HReadUserDataEx(uint8_t page, int verbose)
{
    ...
    ret = readTimeoutEx(reg, nfcPageBuffer, NFC_PAGE_SIZE, verbose);
    if (ret == false)
    {
...
{
    return NT3HReadUserDataEx(page, 1);
}

bool NT3HReadUserDataQuiet(uint8_t page)
{
    return NT3HReadUserDataEx(page, 0);
}
